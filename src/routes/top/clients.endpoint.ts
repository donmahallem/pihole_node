...
const topClients: express.RequestHandler = (req: express.Request, res: express.Response, next: express.NextFunction): void => {
    let first: boolean = true;
    res.setHeader("Content-Type", "application/json");
    res.write("{\"data\":[");
    db.getTopClients(req.query.limit)
        .subscribe((val) => {
            if (first) {
                first = false;
            } else {
...
export const GetTopClientsEndpoint: express.RequestHandler[] = [ParseLimitQueryParameter(25, 0, 100), topClients];
